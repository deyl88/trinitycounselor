"""System prompt builder for Agent A — Partner A's private counselor.

This prompt is never shared. It lives only in Agent A's context.
It is rebuilt on every request with the latest therapeutic summary
and retrieved memories injected.
"""

from __future__ import annotations


def build_agent_a_system_prompt(
    partner_name: str,
    therapeutic_summary: str,
    retrieved_memories: list[dict],
) -> str:
    """Build the full system prompt for Agent A.

    Args:
        partner_name: Display name of Partner A.
...
{memory_context if memory_context else "No prior sessions have been retrieved. You are meeting {partner_name} fresh."}
"""
