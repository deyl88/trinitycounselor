...
function RecCard({ rec, onAct }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontSize: 15, fontWeight: 800, color: C.purple }}>
              {Math.round(rec.style_fit_score * 100)}%
            </span>
          </div>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontSize: 15, fontWeight: 800, color: C.pink }}>
      ...
      {rec.tip_text && (
        <div style={{
          background: 'rgba(199,125,255,0.08)', borderLeft: `3px solid ${C.purple}`,
          borderRadius: '0 12px 12px 0', padding: '10px 14px', marginBottom: 16,
          fontSize: 13, color: '#D4B8FF', lineHeight: 1.6,
        }}>
          {rec.tip_text}
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button
          ...
          disabled={acting}
          style={{
            flex: 2, background: C.grad, border: 'none',
            borderRadius: 14, color: '#fff', padding: '11px',
...
export default function ForYou() {
  ...
  const tabStyle = active => ({
    flex: 1, padding: '9px', fontSize: 13, fontWeight: active ? 700 : 500,
    background: active ? C.grad : 'transparent',
    border: active ? 'none' : `1px solid ${C.border}`,
    borderRadius: 12, color: active ? '#fff' : C.muted, cursor: 'pointer',
  })
  ...
  return (
    <div style={{ padding: '28px 16px 16px', background: C.bg, minHeight: '100%' }}>
      ...
      {loading && (
        <div style={{ textAlign: 'center', color: C.muted, padding: 48 }}>Loading...</div>
      )}
      ...
      {!loading && display.length === 0 && (
        <div style={{ textAlign: 'center', padding: '48px 20px', color: C.muted }}>
          ...
          <p style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 8 }}>
            {tab === 'pending' ? 'All caught up!' : 'Nothing saved yet'}
          </p>
          <p style={{ fontSize: 13 }}>
            {tab === 'pending'
              ? 'Add trending songs in the Trends tab to get recs.'
              : 'Save songs you want to cover from New.'}
          </p>
        </div>
      )}

      {display.map(rec => (
        <RecCard key={rec.id} rec={rec} onAct={dismiss} />
      ))}
    </div>
  )
}
